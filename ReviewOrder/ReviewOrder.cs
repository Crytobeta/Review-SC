...
{
    public class ReviewOrder : Framework.SmartContract
    {
        ...
        public static Object Main(string operation, params object[] args)
        {
            ...
            {
                if (operation == "query")                   return QueryReview((string)args[0]);
                else if (operation == "create")             return CreateReview((string)args[0], (string)args[1]);
                else if (operation == "createMultiple")     return CreateReviewMultiple(args);
                else if (operation == "putMultiple")        return PutReviewMultiple(args);
                else if (operation == "update")             return UpdateReview((string)args[0], (string)args[1]);
        ...
        /**
         * Store multiple reviews data to Storage
         */
        private static object CreateReviewMultiple(object[] args)
        {
            int len = args.Length;
            if (len % 2 != 0)
            {
        ...
        /**
         * Put multiple reviews data to Storage
         */
        private static object PutReviewMultiple(object[] args)
        {
        ...
        */
        private static object UpdateReview(string domain, string value)
        {
            if (!Runtime.CheckWitness(Owner))
            {
                return false;
        ...
        /**
        * Store a review data to Storage
        */
        private static object CreateReview(string domain, string value)
        {
            if (!Runtime.CheckWitness(Owner))
            {
                return false;
        ...
        /**
        * Query review data from Storage
        */
        private static object QueryReview(string domain)
        {
            byte[] v = Storage.Get(Storage.CurrentContext, domain);
            if (v == null) return false;
